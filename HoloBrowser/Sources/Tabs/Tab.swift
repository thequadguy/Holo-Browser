...
public final class Tab: Identifiable, ObservableObject {
    ...
    @Published public private(set) var lastAccessedDate: Date = Date()
    @Published public private(set) var previewSnippet: String? = nil

    /// Extracted host domain name from current URL.
    public var domainHost: String? {
        return url?.host
    }

    /// Domain-aware SF Symbol icon for tab visualization.
    public var faviconSymbol: String {
        if state == .suspended { return "moon.fill" }
        guard let host = domainHost?.lowercased() else { return "globe" }
        if host.contains("github.com") { return "doc.text" }
        if host.contains("apple.com") || host.contains("swift.org") { return "swift" }
        if host.contains("youtube.com") || host.contains("vimeo.com") { return "play.tv.fill" }
        if host.contains("bank") || host.contains("chase") || host.contains("paypal") { return "lock.shield.fill" }
        if host.contains("wikipedia.org") { return "book.fill" }
        return "globe"
    }

    public func touchAccessTime() {
    ...
    public var isPrivate: Bool {
        return !websiteDataStore.isPersistent
    }
    ...
    public func restoreIfNeeded() -> HoloWebView? {
        ...
        ContentBlockingManager.shared.applyRules(to: config)
        if let script = FingerprintProtectionManager.shared.protectionScript() {
