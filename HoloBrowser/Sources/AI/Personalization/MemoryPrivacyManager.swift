...
public enum MemoryPrivacyManager {
    ...
    @MainActor
    public static func clearAllMemories() {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first ?? URL(fileURLWithPath: NSTemporaryDirectory())
        let memoryFile = appSupport.appendingPathComponent("HoloBrowser/holomind_memories.json")
        try? FileManager.default.removeItem(at: memoryFile)
    }
    
    @MainActor
    public static func exportMemoriesJSON() -> String {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first ?? URL(fileURLWithPath: NSTemporaryDirectory())
        let memoryFile = appSupport.appendingPathComponent("HoloBrowser/holomind_memories.json")
        if let data = try? Data(contentsOf: memoryFile), let str = String(data: data, encoding: .utf8) {
            return str
        }
        return "[]"
    }
