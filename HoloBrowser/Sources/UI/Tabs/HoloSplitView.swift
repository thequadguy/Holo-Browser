    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                Menu {
                    ForEach(tabManager.tabs.filter { $0.id != tab.id }) { tabItem in
                        Button(tabItem.title.isEmpty ? "New Tab" : tabItem.title) {
                            tabManager.replaceSplitTab(pane: pane, newTabID: tabItem.id)
                        }
                    }
                } label: {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 4)
                .stroke(
                    isActivePane
                        ? AnyShapeStyle(HoloTheme.Palette.glassBorderGradient)
